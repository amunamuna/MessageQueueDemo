...
public class MessageSenderServiceImpl implements MessageSenderService{
  ...
  public CommonResult sendMessage(String from ,String to,String message) {
    ...
    if(commonResult == null){
      ...
      if(vo != null){
        LogConsole.info("认证成功,获取ta:"+vo.getTa()+",开始用ka解密");
        ...
        if(cerTicketVo != null){
          LogConsole.info("通过ka将ta成功解密,开始封装队列消息");
          ...
          //给队列发送消息
          amqpTemplate.convertAndSend(QueueEnum.QUEUE_LOG.getExchange(), QueueEnum.QUEUE_LOG.getRouteKey(), JSON.toJSONString(encodeData));
          log.info("向mq发送消息:"+JSON.toJSONString(encodeData));
